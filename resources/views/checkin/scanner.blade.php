	...
	qrScanner.start(
		...
		async (decodedText) => {
			...
			if (data.status === 'success') {
				showResult('success', '✅ Acceso permitido<br>' + data.email);
			} else if (data.status === 'used') {
				showResult('warning', '⚠️ Boleto ya utilizado<br>' + data.used_at);
			} else {
				showResult('danger', '❌ ' + data.message);
			}

			// pausa breve para evitar doble lectura
			await qrScanner.pause();
			setTimeout(() => qrScanner.resume(), 2500);
		}
	);
</script>
@endsection
